...
export default function Settings() {
  ...
  const [form, setForm] = useState({
    libraryName: "", totalSeats: "", whatsappNumber: "", phone: "", address: "", city: "", state: "",
  ...
  useEffect(() => {
    if (config) {
      setForm({
        ...
        whatsappNumber: (config as any).whatsappNumber || "", phone: (config as any).phone || "",
        address: (config as any).address || "", city: (config as any).city || "", state: (config as any).state || "",
  ...
  const handleSaveLibraryInfo = () => {
    ...
    handleUpdate({
      ...
      whatsappNumber: form.whatsappNumber.trim() || undefined, phone: form.phone.trim() || undefined,
      address: form.address.trim() || undefined, city: form.city.trim() || undefined,
      state: form.state.trim() || undefined, ownerName: form.ownerName.trim() || undefined,
    ...
    <div className="space-y-6 max-w-4xl mx-auto page-enter">
      <div>
        <h1 className="text-2xl font-bold tracking-tight text-foreground flex items-center gap-2">
          <div className="p-2 rounded-xl bg-indigo-50"><SettingsIcon className="w-5 h-5 text-indigo-600" /></div>
          Settings
        </h1>
        <p className="text-sm text-muted-foreground mt-0.5 ml-[52px]">Configure your library information</p>
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-base">
            <div className="p-1.5 rounded-lg bg-indigo-50"><Building2 className="w-4 h-4 text-indigo-600" /></div>
            Library Information
          </CardTitle>
          <CardDescription>Update your library details shown on the public website and used for WhatsApp enquiries</CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          <div className="space-y-1.5">
            ...
            <textarea id="description" rows={3} placeholder="Describe your library for public visitors..."
          ...
          <div className="space-y-1.5">
            ...
            <textarea id="facilities" rows={2} placeholder="AC, High Speed WiFi, RO Water, Discussion Room"
              value={form.facilities} onChange={(e) => setForm((f) => ({ ...f, facilities: e.target.value }))}
              className="w-full rounded-lg border border-input bg-transparent px-3 py-2 text-sm shadow-sm placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring resize-none" />
            <p className="text-xs text-muted-foreground">These facilities will be displayed on your public website.</p>
          </div>

          <div className="space-y-1.5">
            <Label htmlFor="googleMapsLink" className="text-sm font-medium flex items-center gap-1.5">
              <MapPin className="w-3.5 h-3.5 text-indigo-500" /> Google Maps Link
              <span className="text-xs text-muted-foreground font-normal ml-1">(optional)</span>
            </Label>
            <Input id="googleMapsLink" type="url" placeholder="https://maps.google.com/..."
              value={form.googleMapsLink} onChange={(e) => setForm((f) => ({ ...f, googleMapsLink: e.target.value }))} />
            <p className="text-xs text-muted-foreground">Paste your Google Maps share link here — it will display as a clickable map on your public website.</p>
          </div>

          <Button onClick={handleSaveLibraryInfo} disabled={updateConfig.isPending} className="w-full sm:w-auto shadow-sm">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-base">
            <div className="p-1.5 rounded-lg bg-indigo-50"><Images className="w-4 h-4 text-indigo-600" /></div>
            Website Photos
          </CardTitle>
          <CardDescription>Upload photos shown on your public library page — logo, hero cover, and gallery slideshow.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-8">
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center gap-4">
              <div className="w-20 h-20 rounded-xl border-2 border-dashed border-border flex items-center justify-center bg-muted/20 overflow-hidden cursor-pointer hover:border-primary transition-colors"
              ...
              <div className="space-y-1.5">
                <Button variant="outline" size="sm" onClick={() => logoRef.current?.click()} disabled={photoUploading}>
          ...
          <div className="space-y-3 pt-4 border-t border-border/60">
            ...
            <div className="relative w-full h-40 rounded-xl border-2 border-dashed border-border flex items-center justify-center bg-muted/20 overflow-hidden cursor-pointer hover:border-primary transition-colors"
              onClick={() => coverRef.current?.click()} title="Click to upload cover image">
              {coverPreview ? <img src={coverPreview} alt="Cover" loading="lazy" decoding="async" className="w-full h-full object-cover" />
                : <div className="flex flex-col items-center gap-2 text-muted-foreground"><ImagePlus className="w-8 h-8" /><span className="text-sm">Click to upload cover photo</span></div>}
              {coverPreview && (
                <div className="absolute inset-0 bg-black/30 opacity-0 hover:opacity-100 transition-opacity flex items-center justify-center">
                  <span className="text-white text-sm font-semibold bg-black/50 px-3 py-1 rounded-full"><Upload className="w-3.5 h-3.5 inline mr-1" />Replace</span>
                </div>
              )}
            </div>
          ...
          <div className="space-y-3 pt-4 border-t border-border/60">
            <div className="flex items-center justify-between">
              ...
              <Button variant="outline" size="sm" onClick={() => galleryRef.current?.click()} disabled={photoUploading}>
            ...
            {galleryPreviews.length === 0 ? (
              <div className="w-full h-32 rounded-xl border-2 border-dashed border-border flex flex-col items-center justify-center gap-2 text-muted-foreground cursor-pointer hover:border-primary transition-colors bg-muted/10"
              ...
              <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-2">
                {galleryPreviews.map((src, i) => (
                  <div key={i} className="relative group aspect-square rounded-xl overflow-hidden border border-border/60">
                    ...
                    <button onClick={() => removeGalleryImage(i)} className="absolute top-1 right-1 w-5 h-5 rounded-full bg-black/70 text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-600" title="Remove"><X className="w-3 h-3" /></button>
                    <button onClick={() => { setReplaceIndex(i); galleryReplaceRef.current?.click(); }} className="absolute top-1 left-1 px-2 h-5 rounded-full bg-black/70 text-white text-[10px] font-semibold flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity hover:bg-blue-600" title="Replace">Replace</button>
                  </div>
                ))}
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-base">
            <div className="p-1.5 rounded-lg bg-indigo-50"><Palette className="w-4 h-4 text-indigo-600" /></div>
        ...
        <CardContent className="space-y-6">
          <div className="flex items-center justify-between">
            <div><Label className="text-base">Theme</Label><p className="text-sm text-muted-foreground">Toggle between Light and Dark mode globally.</p></div>
            <ThemeToggle />
          </div>
          <div className="flex items-center justify-between pt-4 border-t border-border/60">
            <div><Label className="text-base">Language</Label><p className="text-sm text-muted-foreground">Set your preferred default language.</p></div>
            <LanguageToggle />
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent className="space-y-6">
          <div className="flex items-center justify-between p-4 bg-muted/20 rounded-xl border border-border/60">
            <div className="space-y-0.5">
              ...
              <p className="text-sm text-muted-foreground">When active, your library profile is visible to the public.</p>
            </div>
            <Switch checked={(config as any)?.websiteEnabled ?? true} onCheckedChange={(checked) => handleUpdate({ websiteEnabled: checked })} />
          </div>

          <div className="space-y-3">
            <Label className="text-sm font-medium">Website URL / Slug</Label>
            <div className="flex flex-col sm:flex-row gap-3">
              <div className="flex items-center flex-1 rounded-lg border bg-muted/20 px-3">
                <span className="text-muted-foreground text-sm select-none truncate">{baseUrl}/library/</span>
                <Input className="border-0 bg-transparent focus-visible:ring-0 focus-visible:ring-offset-0 px-1"
                  defaultValue={(config as any)?.websiteSlug || ""} placeholder="your-library-slug"
                  onChange={(e) => setSlugInput(e.target.value)} />
              </div>
              <Button onClick={() => handleUpdate({ websiteSlug: slugInput || (config as any)?.websiteSlug })} className="shadow-sm">Save Slug</Button>
            </div>
          </div>
              ...
              {WEBSITE_THEMES.map((wt) => {
                ...
                return (
                  <button key={wt.id} onClick={() => handleUpdate({ websiteTheme: wt.id }, `✨ Theme changed to "${wt.name}"!`)}
                    ...
                    <div style={{
                      position: "relative", width: "100%", height: "3.5rem", borderRadius: "0.625rem", overflow: "hidden",
                      border: isActive ? `2.5px solid ${wt.vars.accent}` : isHovered ? `2px solid ${wt.vars.accent}99` : `2px solid transparent`,
                      boxShadow: isActive ? `0 0 0 3px ${wt.vars.accent}33` : isHovered ? `0 4px 12px ${wt.vars.accent}30` : "0 1px 4px rgba(0,0,0,0.18)",
                      transform: isHovered ? "translateY(-1px)" : "none", transition: "all 0.15s ease", display: "flex",
                    }}>
                      ...
                      {isActive && (
                        <div style={{
                          ...
                          width: "1.4rem", height: "1.4rem", borderRadius: "50%", background: "rgba(255,255,255,0.95)",
                    ...
                    <div style={{ marginTop: "0.4rem", fontSize: "0.7rem", fontWeight: isActive ? 700 : 500, color: isActive ? "hsl(var(--foreground))" : "hsl(var(--muted-foreground))", lineHeight: 1.3, paddingLeft: "0.1rem", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                      {wt.emoji} {wt.name}
                    </div>
                  </button>
                );
          ...
          <div className="flex flex-wrap gap-3 pt-2">
            <Button variant="secondary" className="flex-1 sm:flex-none" onClick={() => copyLink((config as any)?.websiteSlug || "")} disabled={!(config as any)?.websiteSlug}>
              <Copy className="w-4 h-4 mr-2" /> Copy Link
            </Button>
            <Button variant="outline" className="flex-1 sm:flex-none" onClick={() => openWebsite((config as any)?.websiteSlug || "")} disabled={!(config as any)?.websiteSlug || !(config as any)?.websiteEnabled}>
