...
function buildWhatsAppMessage(student) {
  ...
  const daysOverdue = student.nextDueDate ? differenceInDays(/* @__PURE__ */ new Date(), new Date(student.nextDueDate)) : 0;
  const dueInfo = student.nextDueDate ? `due on ${format(new Date(student.nextDueDate), "d MMM yyyy")}` : "overdue";
...
function Fees() {
  ...
  const handleSendAll = (students) => {
    if (students.length === 0) {
      toast.info("No students to remind");
      return;
    }
    let sent = 0;
  ...
  return <div className="space-y-6 max-w-7xl mx-auto page-enter">
    ...
    /* Reminder banner */
      ...
      {!overdueLoading && overdueList.length > 0 && <div className="flex items-center justify-between gap-4 rounded-xl border border-red-200 bg-red-50 px-5 py-4">
          ...
          <Button className="bg-emerald-600 hover:bg-emerald-700 shrink-0 shadow-sm" onClick={() => handleSendAll(overdueList)}>
        ...
        <TabsContent value="overdue">
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              ...
              {!overdueLoading && overdueList.length > 0 && <Button
    variant="outline"
    size="sm"
    className="border-emerald-200 text-emerald-600 hover:bg-emerald-50"
    onClick={() => handleSendAll(overdueList)}
  >
                  <Send className="w-3.5 h-3.5 mr-1.5" /> Send All ({overdueList.length})
                </Button>}
            </CardHeader>
            <CardContent>
              {overdueLoading ? <Skeleton className="h-64 w-full" /> : <FeeTable students={overdueList} isOverdue={true} />}
            </CardContent>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="duesToday">
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              ...
              {!duesLoading && dueList.length > 0 && <Button
    variant="outline"
    size="sm"
    className="border-emerald-200 text-emerald-600 hover:bg-emerald-50"
    onClick={() => handleSendAll(dueList)}
  >
                  <Send className="w-3.5 h-3.5 mr-1.5" /> Remind All ({dueList.length})
                </Button>}
            </CardHeader>
            <CardContent>
              {duesLoading ? <Skeleton className="h-64 w-full" /> : <FeeTable students={dueList} isOverdue={false} />}
            </CardContent>
          </Card>
        </TabsContent>
      </Tabs>
...
function FeeTable({ students, isOverdue }) {
  const queryClient = useQueryClient();
  const [payingStudent, setPayingStudent] = useState(null);
  const [remindedIds, setRemindedIds] = useState(/* @__PURE__ */ new Set());
  const handleRemind = (student) => {
    if (openWhatsApp(student)) {
      setRemindedIds((prev) => /* @__PURE__ */ new Set([...prev, student.id]));
      toast.success(`WhatsApp opened for ${student.name}`);
    }
  };
  if (students.length === 0) {
    return <div className="flex flex-col items-center justify-center py-12 text-muted-foreground gap-2">
  ...
  return <>
    ...
    return <TableRow key={student.id} className="hover:bg-muted/30 transition-colors">
                  <TableCell>
                    <div className="flex items-center gap-2.5">
                      ...
                      <div className="min-w-0">
                        ...
                        {student.shifts && student.shifts.length > 0 && <div className="flex gap-1 mt-0.5 flex-wrap">
                            {student.shifts.map((shift) => <span key={shift} className="text-[10px] px-1 py-0.5 rounded bg-indigo-50 text-indigo-600 font-medium capitalize">{shift}</span>)}
                          </div>}
                      </div>
                    </div>
                  </TableCell>
                  ...
                  {isOverdue && <TableCell>
                      {student.nextDueDate ? <span className="text-destructive font-semibold text-sm">
                          {differenceInDays(/* @__PURE__ */ new Date(), new Date(student.nextDueDate))} days
                        </span> : "\u2014"}
                    </TableCell>}
