...
import { Check, UploadCloud, MapPin, Building, Image as ImageIcon, Settings, CheckCircle2 } from "lucide-react";
...
export default function SetupWizard() {
  ...
  const { data: config } = useGetLibraryConfig({ query: { enabled: !!token } as any });
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>, field: "logoFile" | "galleryFiles") => {
          ...
          {steps.map((s, i) => {
            ...
            return (
              <div key={i} className="flex flex-col items-center gap-2 relative z-10 flex-1">
                ...
                {i < steps.length - 1 && (
                  <div className={`absolute top-5 left-[50%] right-[-50%] h-[2px] -z-10 ${step > i + 1 ? 'bg-primary' : 'bg-border'}`} />
                )}
              </div>
            );
          })}
                  ...
                  <div className="space-y-6">
                    <div>
                      ...
                      <div className="flex items-center gap-4">
                        {formData.logoFile && (
                          <ObjectUrlImage file={formData.logoFile} alt="Logo" className="w-20 h-20 rounded-md object-cover border" />
                        )}
                    ...
                    <div>
                      ...
                      {formData.galleryFiles.length < 6 && (
                        <Input type="file" accept="image/*" multiple onChange={(e) => handleFileChange(e, 'galleryFiles')} />
                      )}
                    </div>
                  </div>
                )}
