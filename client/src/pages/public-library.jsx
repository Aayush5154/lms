...
function PublicLibrary() {
  ...
  useEffect(() => {
    ...
    return () => {
      document.getElementById("pub-lib-global")?.remove();
    };
  }, []);
  ...
  const theme = getTheme(library.websiteTheme);
  const t = theme.vars;
  const cssVars = {
    "--bg": t.bg,
  ...
  const getFacilityIcon = (name, size = 24) => {
  ...
  const backendFacilities = library.facilities && library.facilities.length > 0 ? library.facilities : [
    "High-Speed WiFi",
    "Air Conditioned",
    "Power Backup",
    "RO Water",
  ...
  const topFeatures = backendFacilities.slice(0, 6).map((fac) => ({
    icon: getFacilityIcon(fac, 24),
    title: fac,
    desc: getFacilityDesc(fac)
  }));
  const gridFacilities = backendFacilities.map((fac) => ({
    icon: getFacilityIcon(fac, 28),
    title: fac
  }));
  const plans = library.membershipPlans && library.membershipPlans.length > 0 ? library.membershipPlans : [
    { name: "Basic", price: `\u20B9${library.monthlyFeeDefault || 1e3}`, desc: "Perfect for part-time learners and flexible schedules.", features: ["Flexible Seating", "High-Speed WiFi", "RO Water", "Standard Support"] },
    { name: "Premium", price: `\u20B9${(library.monthlyFeeDefault || 1e3) * 2}`, desc: "For dedicated students & pros who need the absolute best.", features: ["Reserved Cabin Seat", "24/7 Access", "Unlimited Coffee", "Discussion Room", "Locker Included"], recommended: true },
    { name: "Standard", price: `\u20B9${Math.round((library.monthlyFeeDefault || 1e3) * 1.5)}`, desc: "The balanced everyday plan for consistent learners.", features: ["Dedicated Desk", "High-Speed WiFi", "RO Water", "Power Backup"] }
  ];
  return <div style={{ ...cssVars, backgroundColor: "var(--bg)", color: "var(--text)" }} className="min-h-screen relative font-sans">
      ...
      <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? "bg-[var(--card)] shadow-md py-3" : "bg-[var(--card)] py-4"}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center">
          <div className="flex items-center gap-3 cursor-pointer max-w-[70%]" onClick={() => scrollTo("home")}>
            {library.logoUrl ? <img src={library.logoUrl} alt="Logo" className="w-10 h-10 sm:w-12 sm:h-12 object-contain" /> : <div className="flex items-center gap-2 overflow-hidden">
          ...
          <nav className="hidden md:flex items-center gap-8">
            {["Home", "Facilities", "Gallery", "Location", "Contact"].map((item) => <button key={item} onClick={() => scrollTo(item.toLowerCase())} className="text-[15px] font-semibold hover:text-[var(--accent)] transition-colors">
                {item}
              </button>)}
          </nav>
    ...
    /* MOBILE MENU */
      ...
      <AnimatePresence>
        {mobileMenuOpen && <motion.div initial={{ opacity: 0, x: "100%" }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: "100%" }} className="fixed inset-0 z-[60] bg-[var(--card)] flex flex-col pt-24 px-6">
            <button className="absolute top-6 right-6 p-2" onClick={() => setMobileMenuOpen(false)}>
              <X size={32} />
            </button>
            <div className="flex flex-col gap-6 text-xl font-bold">
              {["Home", "Facilities", "Gallery", "Location", "Contact"].map((item) => <button key={item} onClick={() => scrollTo(item.toLowerCase())} className="text-left py-2 border-b border-[var(--card-border)] hover:text-[var(--accent)]">
                  {item}
                </button>)}
    ...
    /* Background Image with Fade */
        ...
        <div className="absolute top-0 right-0 w-full lg:w-[65%] h-full z-0 overflow-hidden">
          <div className="absolute inset-0 bg-gradient-to-r from-[var(--bg)] via-[var(--bg)]/80 to-transparent z-10 lg:block hidden" />
          <div className="absolute inset-0 bg-[var(--bg)]/85 sm:bg-[var(--bg)]/70 z-10 lg:hidden block" />
    ...
    /* Active Learners */
            ...
            <div className="flex items-center gap-4">
              <div className="flex -space-x-3">
                {[1, 2, 3, 4].map((i) => <img key={i} src={`https://i.pravatar.cc/100?img=${i + 10}`} alt="Student" className="w-12 h-12 rounded-full border-2 border-[var(--bg)] object-cover" />)}
              </div>
              <div>
    ...
    /* Floating Features Bar */
        ...
        <div className="absolute bottom-0 left-1/2 transform -translate-x-1/2 translate-y-1/2 w-full max-w-6xl px-4 z-30 hidden lg:block">
          <div className="bg-[var(--card)] rounded-2xl shadow-xl border border-[var(--card-border)] p-6 flex justify-between items-center divide-x divide-[var(--card-border)]">
            {topFeatures.map((feat, i) => <div key={i} className="flex flex-col items-center justify-center flex-1 px-2 text-center gap-2">
      ...
      <section id="facilities" className="py-24 lg:py-32 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto bg-[var(--bg)]">
        <div className="flex flex-col lg:flex-row gap-12 lg:gap-20">
          ...
          <div className="lg:w-2/3 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 sm:gap-4">
            {gridFacilities.map((fac, i) => <div key={i} className="bg-[var(--card)] border border-[var(--card-border)] rounded-xl p-4 sm:p-6 flex flex-col items-center justify-center text-center gap-3 sm:gap-4 hover:border-[var(--accent)] transition-colors shadow-sm cursor-default">
  ...
  ].map((stat, i) => <div key={i} className="flex flex-col items-center lg:items-start text-center lg:text-left gap-2 sm:gap-3">
  ...
  >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
          {(showFullGallery ? galleryImgs : galleryImgs.slice(0, 6)).map((img, i) => <div key={i} className="aspect-[4/3] rounded-xl overflow-hidden cursor-pointer group relative" onClick={() => setLightboxImg(img)}>
    ...
    return <div key={i} className={`bg-[var(--card)] rounded-2xl p-8 relative flex flex-col h-full ${isPopular ? "border-2 shadow-2xl md:-translate-y-4" : "border shadow-md"}`} style={{ borderColor: isPopular ? "var(--accent)" : "var(--card-border)" }}>
                  {isPopular && <div className="absolute -top-3 left-1/2 transform -translate-x-1/2 px-4 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider text-white shadow-md" style={{ backgroundColor: "var(--accent)" }}>
                      Most Popular
                    </div>}
                  ...
                  <ul className="space-y-4 mb-8 flex-1">
                    {plan.features.map((feat, j) => <li key={j} className="flex items-start gap-3 text-[var(--text)] text-sm font-medium">
      ...
      <section id="location" className="py-24 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        ...
        <div className="flex flex-col lg:flex-row gap-12 bg-[var(--card)] p-4 sm:p-8 rounded-3xl border border-[var(--card-border)] shadow-sm">
          <div className="lg:w-1/2 rounded-2xl overflow-hidden min-h-[300px] border border-[var(--card-border)]">
            <img src="/google-maps-placeholder.jpeg" alt="Location Map" className="w-full h-full object-cover" onError={(e) => {
    e.currentTarget.src = "https://images.unsplash.com/photo-1524661135-423995f22d0b?auto=format&fit=crop&q=80";
  }} />
          </div>
  ...
  ].map((test, i) => <div key={i} className="flex-1 min-w-[280px] sm:min-w-[300px] snap-center bg-[var(--card)] p-6 sm:p-8 rounded-2xl border border-[var(--card-border)] shadow-sm">
      ...
      <footer id="contact" className="pt-20 pb-10 px-4 sm:px-6 lg:px-8" style={{ backgroundColor: "var(--footer-bg)", color: "var(--footer-text)" }}>
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-12 mb-16">
            <div className="lg:col-span-2">
              <div className="flex items-center gap-3 mb-6">
                {library.logoUrl ? <img src={library.logoUrl} alt="Logo" className="w-10 h-10 object-contain" /> : <BookOpen size={32} style={{ color: "var(--accent)" }} />}
            ...
            <div>
              ...
              <ul className="space-y-3 text-sm font-medium opacity-70">
                {backendFacilities.slice(0, 5).map((fac, i) => <li key={i}>{fac}</li>)}
              </ul>
            </div>
          ...
          <div className="pt-8 border-t border-white/10 flex flex-col md:flex-row items-center justify-between gap-4 text-xs font-medium opacity-60">
            <div>&copy; {(/* @__PURE__ */ new Date()).getFullYear()} {library.libraryName}. All rights reserved.</div>
    ...
    /* LIGHTBOX */
      ...
      <AnimatePresence>
        {lightboxImg && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] bg-black/95 flex items-center justify-center p-4 backdrop-blur-sm" onClick={() => setLightboxImg(null)}>
