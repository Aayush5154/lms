...
export default function Login() {
  ...
  const onSubmit = (values: LoginFormValues) => {
    loginMutation.mutate({ data: values }, {
      onSuccess: (res) => { toast.success("Login successful"); login(res.token, res.admin); },
      onError: (err: any) => { toast.error(err?.data?.message || "Invalid email or password"); },
    });
  };
      ...
      <div className="flex-1 hidden lg:flex items-center justify-center p-12 relative overflow-hidden login-hero">
        ...
        <div className="relative z-10 max-w-lg">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
            ...
            <h1 className="text-4xl font-bold mb-4 tracking-tight text-white">Library Operating System</h1>
      ...
      <div className="flex-1 flex items-center justify-center p-6 sm:p-12 relative bg-background">
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.4 }} className="w-full max-w-[400px]">
          <div className="lg:hidden flex items-center justify-center mb-8">
             <div className="w-12 h-12 rounded-xl flex items-center justify-center login-hero-badge">
            ...
            <CardContent>
              <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                  <FormField control={form.control} name="email"
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl><Input placeholder="admin@example.com" {...field} className="h-11" /></FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
                  />
                  ...
                  />
                  <Button type="submit" className="w-full h-11 text-base font-medium mt-2 shadow-sm" disabled={loginMutation.isPending}>
