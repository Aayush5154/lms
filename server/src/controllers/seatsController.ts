...
export async function listSeats(req: AuthRequest, res: Response): Promise<void> {
  try {
    ...
    let seats = Array.from({ length: totalSeats }, (_, i) => {
      ...
      return { seatNumber, isOccupied: !!student, status: seatStatus, student: student ? formatStudent(student as any) : undefined };
    });
...
export async function updateSeatsConfig(req: AuthRequest, res: Response): Promise<void> {
  try {
