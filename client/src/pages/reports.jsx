...
function Reports() {
  ...
  const occupancyData = useMemo(() => {
    return [
      { name: "Occupied", value: occupancy?.occupied || 0, fill: "#4f46e5" },
      // indigo-600
      { name: "Available", value: occupancy?.vacant || 0, fill: "#64748b" }
      // slate-500
    ];
  }, [occupancy]);
  ...
  const totalSeatsOccupiedRate = occupancy?.total ? Math.round(occupancy.occupied / occupancy.total * 100) : 0;
  ...
  const exportExcel = () => {
    ...
    XLSX.writeFile(wb, `Library_Report_${format(/* @__PURE__ */ new Date(), "yyyy_MM_dd")}.xlsx`);
  };
  const exportCSV = () => {
    const wsData = getReportData();
    if (wsData.length === 0) return;
    ...
    link.href = url;
    link.setAttribute("download", `Library_Report_${format(/* @__PURE__ */ new Date(), "yyyy_MM_dd")}.csv`);
  ...
  const exportPDF = () => {
    ...
    doc.setFontSize(12);
    doc.text(`Generated on: ${format(/* @__PURE__ */ new Date(), "dd MMM yyyy")}`, 14, 32);
    doc.setFontSize(14);
    ...
    doc.save(`Library_Report_${format(/* @__PURE__ */ new Date(), "yyyy_MM_dd")}.pdf`);
  };
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-x-12 gap-y-10">
            <div className="flex flex-col">
              ...
              <div className="h-[250px] w-full">
                {monthlyCollectionData.length > 0 ? <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={monthlyCollectionData} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
                      ...
                      <RechartsTooltip cursor={{ fill: "hsl(var(--muted))" }} contentStyle={{ borderRadius: "8px", border: "none", boxShadow: "0 4px 6px -1px rgb(0 0 0 / 0.1)", background: "hsl(var(--card))", color: "hsl(var(--card-foreground))" }} formatter={(val) => [`\u20B9${val}`, "Collection"]} />
                      <Bar dataKey="value" fill="#6366f1" radius={[4, 4, 0, 0]} maxBarSize={40} />
                    </BarChart>
                  </ResponsiveContainer> : <div className="w-full h-full flex items-center justify-center border-2 border-dashed border-border rounded-xl bg-muted text-muted-foreground">
              ...
              <div className="h-[250px] w-full">
                {monthlyCollectionData.length > 0 ? <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={monthlyCollectionData} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
                      ...
                      <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fontSize: 12, fill: "hsl(var(--muted-foreground))" }} dy={10} />
                      <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 12, fill: "hsl(var(--muted-foreground))" }} tickFormatter={(val) => `\u20B9${val >= 1e3 ? val / 1e3 + "k" : val}`} />
                      <RechartsTooltip contentStyle={{ borderRadius: "8px", border: "none", boxShadow: "0 4px 6px -1px rgb(0 0 0 / 0.1)", background: "hsl(var(--card))", color: "hsl(var(--card-foreground))" }} formatter={(val) => [`\u20B9${val}`, "Collection"]} />
...
function ReportMiniStat({ title, value, icon, trend, trendDown, valueColor = "text-foreground" }) {
