...
function CustomTooltip({ active, payload, label }) {
  ...
  return <div className="bg-card border border-border rounded-lg px-3 py-2 shadow-lg">
  ...
  />
    ...
    /* Monthly Collection Overview */
          ...
          <Card className="card-shadow card-enter card-enter-delay-2 h-[380px] flex flex-col">
            ...
            <CardContent className="flex-1 pb-6 mt-4">
              {revLoading ? <Skeleton className="w-full h-full" /> : <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={revenueData} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
                    <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="hsl(var(--border))" />
                    <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fontSize: 12, fill: "hsl(var(--muted-foreground))" }} dy={10} />
                    <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 12, fill: "hsl(var(--muted-foreground))" }} tickFormatter={(val) => `\u20B9${val >= 1e3 ? val / 1e3 + "k" : val}`} />
    ...
    /* Recent Payments Preview */
          ...
          <Card className="card-shadow card-enter card-enter-delay-4">
            ...
            <CardContent>
              {payLoading ? <Skeleton className="h-[200px] w-full" /> : <div className="overflow-x-auto">
                  <Table>
                    <TableHeader>
                    ...
                    <TableBody>
                      {!recentPayments || recentPayments.length === 0 ? <TableRow>
                          <TableCell colSpan={6} className="h-24 text-center text-muted-foreground">No recent payments</TableCell>
                        </TableRow> : recentPayments.slice(0, 4).map((payment) => <TableRow key={payment.id} className="border-b border-border hover:bg-muted/50 transition-colors">
    ...
    /* Recent Activities */
          ...
          <Card className="card-shadow card-enter card-enter-delay-4">
            ...
            <CardContent>
              {payLoading ? <Skeleton className="h-[200px] w-full" /> : <div className="space-y-6 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-border before:to-transparent">
                  {!recentPayments || recentPayments.length === 0 ? <div className="text-center text-muted-foreground text-sm py-8">No recent activities</div> : recentPayments.slice(0, 4).map((payment, i) => <ActivityItem
    key={payment.id || i}
...
function StatCard({ title, value, icon, trend, trendUp, delay = 0 }) {
  return <Card className={`shadow-sm card-enter`} style={{ animationDelay: `${delay * 60}ms` }}>
...
function QuickAction({ icon, label, bg, border, onClick }) {
  return <div onClick={onClick} className={`flex flex-col items-center justify-center p-4 rounded-xl border ${border} ${bg} cursor-pointer hover:shadow-md transition-all`}>
