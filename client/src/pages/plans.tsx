...
export default function Plans() {
  ...
  const handleAddPlan = () => { setPlans([...plans, { name: "New Plan", price: "0", description: "", features: [], recommended: false }]); };
  const handleRemovePlan = (index: number) => { setPlans(plans.filter((_, i) => i !== index)); };
  const handlePlanChange = (index: number, field: string, value: any) => {
    const newPlans = [...plans]; newPlans[index][field] = value;
    if (field === "recommended" && value === true) { newPlans.forEach((p, i) => { if (i !== index) p.recommended = false; }); }
    setPlans(newPlans);
  };
  const handleAddFeature = (planIndex: number) => { const newPlans = [...plans]; newPlans[planIndex].features.push("New Feature"); setPlans(newPlans); };
  const handleFeatureChange = (planIndex: number, featureIndex: number, value: string) => { const newPlans = [...plans]; newPlans[planIndex].features[featureIndex] = value; setPlans(newPlans); };
  const handleRemoveFeature = (planIndex: number, featureIndex: number) => { const newPlans = [...plans]; newPlans[planIndex].features.splice(featureIndex, 1); setPlans(newPlans); };

  const handleSave = () => {
    const cleanPlans = plans.map(({ _id, ...rest }: any) => ({ name: rest.name, price: rest.price, description: rest.description, features: rest.features, recommended: rest.recommended }));
    updateConfig.mutate({ data: { membershipPlans: cleanPlans } as any }, {
      onSuccess: () => { toast.success("Membership plans saved successfully"); refetch(); },
            ...
            <Card key={i} className={`relative flex flex-col ${plan.recommended ? 'border-primary/40 shadow-md shadow-primary/10' : ''}`}>
              ...
              <CardHeader className="pb-4 pt-6">
                ...
                <div className="space-y-1">
                  ...
                  <Input value={plan.price} onChange={(e) => handlePlanChange(i, "price", e.target.value)} placeholder="e.g. ₹2000/mo"
                ...
                <div className="space-y-2">
                  ...
                  <div className="space-y-2">
                    ...
                    {plan.features.length === 0 && (
                      <div className="text-xs text-muted-foreground text-center py-2 border border-dashed rounded-lg">No facilities added</div>
                    )}
                  </div>
                </div>
