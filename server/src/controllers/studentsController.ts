...
export async function uploadStudentPhoto(req: AuthRequest, res: Response): Promise<void> {
  try {
    const student = await Student.findOne({ _id: req.params["id"], adminId: req.adminId });
    if (!student) { res.status(404).json({ error: "Student not found" }); return; }
...
export async function getStudentPayments(req: AuthRequest, res: Response): Promise<void> {
  try {
