...
import { Copy, ExternalLink, RefreshCw, Building2, Save, ImagePlus, Upload, X, Images } from "lucide-react";
...
function Settings() {
  ...
  const galleryReplaceRef = useRef(null);
  const [replaceIndex, setReplaceIndex] = useState(null);
  useEffect(() => {
    if (config) {
      setForm({
        libraryName: config.libraryName || "",
        totalSeats: String(config.totalSeats || ""),
      ...
      if (config.coverImageUrl) setCoverPreview(config.coverImageUrl);
      if (config.galleryImages?.length) setGalleryPreviews(config.galleryImages);
    }
  }, [config]);
  const handleUpdate = (updates, successMessage = "Settings updated") => {
  ...
  const handleSaveLibraryInfo = () => {
    ...
    handleUpdate({
      ...
      totalSeats: seats,
      whatsappNumber: form.whatsappNumber.trim() || void 0,
      phone: form.phone.trim() || void 0,
      email: form.email.trim() || void 0,
      address: form.address.trim() || void 0,
      city: form.city.trim() || void 0,
      state: form.state.trim() || void 0,
      pincode: form.pincode.trim() || void 0,
      ownerName: form.ownerName.trim() || void 0,
      openingTime: form.openingTime.trim() || void 0,
      closingTime: form.closingTime.trim() || void 0,
      description: form.description.trim() || void 0,
      googleMapsLink: form.googleMapsLink.trim() || void 0,
  ...
  const removeGalleryImage = (idx) => {
    ...
    updateConfig.mutate({ data: { galleryImages: updated } }, {
      ...
      onError: (err) => toast.error(err.response?.data?.error || "Remove failed")
    });
  };
  ...
  const field = (key, label, type = "text", placeholder = "", required = false) => <div className="space-y-1.5">
    ...
    className="bg-transparent border-border shadow-sm h-10"
  />
    </div>;
  ...
  />
    ...
    id="email"
    type="email"
    placeholder="e.g. info@lmsplatform.com"
    value={form.email}
  ...
  />
    ...
    /* Cover / Hero Image Inline */
              ...
              <div className="space-y-3 pt-6 border-t border-border">
                ...
                <div className="relative w-full h-48 rounded-xl border border-border flex flex-col items-center justify-center bg-muted/20 overflow-hidden shadow-sm">
                  {coverPreview ? <img src={coverPreview} alt="Cover" loading="lazy" decoding="async" className="w-full h-full object-cover" /> : <div className="flex flex-col items-center justify-center h-full text-muted-foreground opacity-50"><Images className="w-10 h-10 mb-2" /></div>}
                </div>
                ...
                <Card className="shadow-sm border-border">
                  <CardContent className="p-6 flex flex-col items-center justify-center">
                    <div className="w-24 h-24 rounded-2xl border-2 border-dashed border-border flex items-center justify-center bg-muted/30 mb-6 overflow-hidden">
                      {logoPreview ? <img src={logoPreview} alt="Logo" loading="lazy" decoding="async" className="w-full h-full object-cover" /> : <Building2 className="w-8 h-8 text-indigo-500" />}
                    </div>
    ...
    description="Make library website public"
    checked={config?.websiteEnabled ?? true}
    ...
    /* Gallery */
           ...
           <Card className="card-shadow">
            ...
            <CardContent>
              ...
              {galleryPreviews.length === 0 ? <div
    className="w-full h-40 rounded-xl border-2 border-dashed border-border flex flex-col items-center justify-center gap-2 text-muted-foreground cursor-pointer hover:border-primary transition-colors bg-muted/10"
  ...
  >
                  ...
                  {galleryPreviews.map((src, i) => <div key={i} className="relative group aspect-square rounded-xl overflow-hidden border border-border shadow-sm">
  ...
  }} className="absolute top-2 left-2 px-2.5 h-6 rounded-full bg-black/70 text-white text-[10px] font-semibold flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity hover:bg-blue-600" title="Replace">Replace</button>
                    </div>)}
                  <div
    className="aspect-square rounded-xl border-2 border-dashed border-border flex items-center justify-center cursor-pointer hover:border-primary transition-colors bg-muted/10"
    onClick={() => galleryRef.current?.click()}
  >
              ...
              <div className="space-y-3">
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  <div className="flex items-center flex-1 rounded-lg border border-border bg-muted/30 px-3 h-11">
                    ...
                    <Input
    className="border-0 bg-transparent focus-visible:ring-0 focus-visible:ring-offset-0 px-1 font-medium"
    defaultValue={config?.websiteSlug || ""}
    placeholder="your-library-slug"
    onChange={(e) => setSlugInput(e.target.value)}
  />
                  </div>
                  <Button onClick={() => handleUpdate({ websiteSlug: slugInput || config?.websiteSlug })} className="shadow-sm btn-press h-11 px-6 bg-indigo-600 hover:bg-indigo-700 text-white">Save Slug</Button>
                </div>
              </div>
              ...
              <div className="space-y-4 pt-4 border-t border-border">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    Choose how your public library page looks to visitors. Current: <span className="font-semibold text-foreground">{getTheme(config?.websiteTheme).name}</span>
                  </p>
                </div>

                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                  {WEBSITE_THEMES.map((wt) => {
    const isActive = (config?.websiteTheme ?? "black-gold") === wt.id;
    const isHovered = hoveredTheme === wt.id;
    return <button
      key={wt.id}
      onClick={() => handleUpdate({ websiteTheme: wt.id }, `\u2728 Theme changed to "${wt.name}"!`)}
    ...
    >
      ...
      boxShadow: "0 2px 8px rgba(0,0,0,0.3)"
              ...
              <div className="flex flex-wrap gap-3 pt-6 border-t border-border">
                <Button variant="secondary" className="flex-1 sm:flex-none btn-press h-10 px-6 font-medium bg-muted hover:bg-muted/80" onClick={() => copyLink(config?.websiteSlug || "")} disabled={!config?.websiteSlug}>
                  <Copy className="w-4 h-4 mr-2" /> Copy Link
                </Button>
                <Button variant="outline" className="flex-1 sm:flex-none btn-press h-10 px-6 font-medium shadow-sm" onClick={() => openWebsite(config?.websiteSlug || "")} disabled={!config?.websiteSlug || !config?.websiteEnabled}>
    ...
    const generated = `${config?.libraryName?.toLowerCase().replace(/[^a-z0-9]+/g, "-")}-${random}`;
...
function ToggleSetting({ label, description, defaultChecked, checked, onCheckedChange }) {
