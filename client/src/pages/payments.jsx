...
function Payments() {
  ...
  return <div className="space-y-6 max-w-7xl mx-auto page-enter">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        {payments.length > 0 && <Badge variant="secondary" className="text-sm px-4 py-1.5 font-semibold bg-emerald-50 text-emerald-700 border-emerald-200 badge-hover card-enter card-enter-delay-1">
      ...
      <Card className="card-shadow card-enter card-enter-delay-2">
        ...
        <CardContent>
          {isLoading ? <div className="space-y-2">{Array.from({ length: 5 }).map((_, i) => <Skeleton key={i} className="h-12 w-full" />)}</div> : payments.length === 0 ? <div className="text-center py-12 text-muted-foreground">
              <Receipt className="w-10 h-10 mx-auto mb-3 opacity-20" />
              <p className="text-sm">No payments found for the selected period</p>
            </div> : <div className="rounded-lg border border-border/60 overflow-hidden">
              <Table>
                <TableHeader>
                ...
                <TableBody>
                  {payments.map((payment) => <TableRow key={payment.id} className="table-row-hover">
                      <TableCell>
    ...
    variant="ghost"
    size="icon"
    className="h-8 w-8 text-muted-foreground hover:text-foreground transition-all duration-150 hover:scale-110"
    title="Print receipt"
