...
function Plans() {
  ...
  useEffect(() => {
    if (config) {
      if (config.membershipPlans && config.membershipPlans.length > 0) {
        setPlans(config.membershipPlans);
      } else {
        setPlans([
          { name: "Basic", price: String(config.monthlyFeeDefault || 1e3), description: "Perfect for part-time learners.", features: ["Flexible Seating", "High-Speed WiFi", "RO Water"], recommended: false },
          { name: "Premium", price: String((config.monthlyFeeDefault || 1e3) * 2), description: "For dedicated students.", features: ["Reserved Cabin Seat", "24/7 Access", "Unlimited Coffee", "Discussion Room"], recommended: true },
          { name: "Standard", price: String(Math.round((config.monthlyFeeDefault || 1e3) * 1.5)), description: "The balanced everyday plan.", features: ["Dedicated Desk", "High-Speed WiFi", "Power Backup"], recommended: false }
        ]);
      }
    }
  }, [config]);
  ...
  const handlePlanChange = (index, field, value) => {
    ...
    if (field === "recommended" && value === true) {
      newPlans.forEach((p, i) => {
        if (i !== index) p.recommended = false;
      });
    }
    setPlans(newPlans);
  };
  ...
  const handleSave = () => {
    const cleanPlans = plans.map(({ _id, ...rest }) => ({ name: rest.name, price: rest.price, description: rest.description, features: rest.features, recommended: rest.recommended }));
    updateConfig.mutate({ data: { membershipPlans: cleanPlans } }, {
      onSuccess: () => {
        toast.success("Membership plans saved successfully");
        refetch();
      },
      onError: (err) => toast.error(err.response?.data?.error || err.message || "Failed to save plans")
    });
  };
  if (isLoading) {
    return <div className="space-y-6 max-w-5xl mx-auto"><Skeleton className="h-10 w-48" /><Skeleton className="h-64 w-full" /></div>;
  }
  return <div className="space-y-6 max-w-5xl mx-auto page-enter">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-foreground flex items-center gap-2">
            <div className="p-2 rounded-xl bg-indigo-50"><Tags className="w-5 h-5 text-indigo-600" /></div>
            Membership Plans
          </h1>
      ...
      {plans.length === 0 ? <Card className="flex flex-col items-center justify-center py-12 text-center bg-muted/10 card-shadow card-enter">
          ...
          {plans.map((plan, i) => <Card key={i} className={`relative flex flex-col card-shadow hover-lift card-enter`} style={{ animationDelay: `${i * 80}ms` }}>
              {plan.recommended && <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-primary text-primary-foreground text-xs font-bold px-3 py-1 rounded-full flex items-center gap-1 shadow-sm badge-hover">
  ...
  />
                ...
                <div className="space-y-2">
                  ...
                  <div className="space-y-2">
                    {plan.features.map((feat, j) => <div key={j} className="flex items-center gap-2">
                    ...
                    {plan.features.length === 0 && <div className="text-xs text-muted-foreground text-center py-2 border border-dashed rounded-lg">No facilities added</div>}
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between pt-4 border-t border-border/60 mt-auto">
                  ...
                  <Switch id={`rec-${i}`} checked={plan.recommended} onCheckedChange={(checked) => handlePlanChange(i, "recommended", checked)} />
                </div>
              </CardContent>
            </Card>)}
        </div>}
