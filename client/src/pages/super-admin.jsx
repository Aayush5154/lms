...
async function superFetch(path, method = "GET", body) {
...
const defaultLibForm = { email: "", password: "", name: "", libraryName: "", totalSeats: "50" };
function SuperAdmin() {
  const qc = useQueryClient();
  const [showCreate, setShowCreate] = useState(false);
  const [form, setForm] = useState(defaultLibForm);
  ...
  const createMutation = useMutation({
    mutationFn: (data) => superFetch("/super/libraries", "POST", data),
    onSuccess: () => {
      toast.success("Library created!");
      setShowCreate(false);
      setForm(defaultLibForm);
      qc.invalidateQueries({ queryKey: ["superAdmins"] });
    },
    onError: (e) => toast.error(e.message)
  });
  const approveMutation = useMutation({
    mutationFn: (id) => superFetch(`/super/libraries/${id}/approve`, "PATCH"),
    onSuccess: () => {
      toast.success("Library approved");
      qc.invalidateQueries({ queryKey: ["superAdmins"] });
    },
    onError: (e) => toast.error(e.message)
  });
  const suspendMutation = useMutation({
    mutationFn: (id) => superFetch(`/super/libraries/${id}/suspend`, "PATCH"),
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => superFetch(`/super/libraries/${id}`, "DELETE"),
    onSuccess: () => {
  ...
  const resetMutation = useMutation({
    mutationFn: ({ id, pwd }) => superFetch(`/super/libraries/${id}/reset-password`, "PATCH", { newPassword: pwd }),
    onSuccess: () => {
  ...
  const [editConfigTarget, setEditConfigTarget] = useState(null);
  const [configForm, setConfigForm] = useState({ theme: "light", websiteEnabled: true, websiteSlug: "" });
  const configMutation = useMutation({
    mutationFn: ({ id, data }) => superFetch(`/super/libraries/${id}/config`, "PUT", data),
    onSuccess: () => {
  ...
  return <div className="space-y-6 max-w-7xl mx-auto page-enter">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2">
            <div className="p-2 rounded-xl bg-indigo-50"><Shield className="w-5 h-5 text-indigo-600" /></div>
            Super Admin
          </h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <StatCard icon={<CheckCircle className="w-5 h-5 text-emerald-600" />} label="Active" value={admins.filter((a) => a.isActive).length} bg="bg-emerald-50" delay={1} />
        <StatCard icon={<XCircle className="w-5 h-5 text-red-500" />} label="Suspended" value={admins.filter((a) => !a.isActive).length} bg="bg-red-50" delay={2} />
      </div>
      ...
      <Card className="card-shadow card-enter card-enter-delay-3">
        ...
        <CardContent>
          {isLoading ? <div className="space-y-2">{Array.from({ length: 3 }).map((_, i) => <div key={i} className="h-14 bg-muted animate-pulse rounded-lg" />)}</div> : <div className="rounded-lg border border-border/60 overflow-hidden">
              <Table>
                <TableHeader>
                ...
                <TableBody>
                  {admins.map((admin) => <TableRow key={admin._id ?? admin.id} className="table-row-hover">
                      <TableCell>
                      ...
                      <TableCell>
                        {admin.isActive ? <Badge className="bg-emerald-50 text-emerald-700 border-emerald-200">Active</Badge> : <Badge variant="destructive">Suspended</Badge>}
                      </TableCell>
                      <TableCell className="text-right">
                        <div className="flex items-center justify-end gap-1">
                          {!admin.isActive ? <Button
    variant="outline"
    size="sm"
    className="h-7 text-xs text-emerald-600 border-emerald-200 hover:bg-emerald-50"
    onClick={() => approveMutation.mutate(admin._id ?? admin.id)}
  ...
  >
    ...
    onClick={() => suspendMutation.mutate(admin._id ?? admin.id)}
  ...
  >
    ...
    variant="ghost"
    size="icon"
    className="h-7 w-7 text-indigo-500 transition-all duration-150 hover:scale-110"
    onClick={() => {
      setResetTarget({ id: admin._id ?? admin.id, name: admin.name });
      setNewPwd("");
    }}
  >
    ...
    variant="ghost"
    size="icon"
    className="h-7 w-7 text-purple-500 transition-all duration-150 hover:scale-110"
    onClick={() => {
  ...
  >
    ...
    variant="ghost"
    size="icon"
    className="h-7 w-7 text-destructive transition-all duration-150 hover:scale-110"
    onClick={() => {
      if (confirm(`Delete ${admin.name}?`)) deleteMutation.mutate(admin._id ?? admin.id);
    }}
  ...
  >
                  ...
                  {admins.length === 0 && <TableRow><TableCell colSpan={5} className="h-24 text-center text-muted-foreground">No libraries yet</TableCell></TableRow>}
                </TableBody>
              </Table>
            </div>}
        </CardContent>
    ...
    setShowCreate(open);
    if (!open) setForm(defaultLibForm);
  }}>
      ...
      <Dialog open={!!editConfigTarget} onOpenChange={(open) => !open && setEditConfigTarget(null)}>
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4 py-2">
            <div className="flex items-center justify-between p-3 border border-border/60 rounded-xl">
              <div className="space-y-0.5">
                <Label>Theme Mode</Label>
                <p className="text-xs text-muted-foreground">Force light or dark mode.</p>
              </div>
              <Button variant="outline" size="sm" onClick={() => setConfigForm((f) => ({ ...f, theme: f.theme === "light" ? "dark" : "light" }))}>
...
function StatCard({ icon, label, value, bg, delay = 0 }) {
  return <Card className="hover-lift card-shadow card-enter" style={{ animationDelay: `${delay * 80}ms` }}>
